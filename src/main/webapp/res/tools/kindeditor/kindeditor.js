...
K.options = {
	...
	htmlTags : {
		font : ['id', 'class', 'color', 'size', 'face', '.background-color','style'],
		span : [
	...
	self.afterCreate(function() {
		...
		function movePastedData() {
			...
			if (self.pasteType === 2) {
				html = html.replace(/(<(?:p|p\s[^>]*)>) *(<\/p>)/ig, '');
				//��ͨճ��
				//html = html.replace(/<table.*?>/gi, "<TABLE border='1' width='100%' style='border-collapse:collapse;'>");
				//html = html.replace(/<td.*?>/gi, "<TD style='border:1px solid #333;'>");
				//html=html.replace(/(<td[^>].>)/g," style='border:1px solid #333;'>")
